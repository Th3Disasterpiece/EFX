...
class PreviewManager:
    ...
    def load_flipbook(self, flipbook_path: str) -> List[QtGui.QPixmap]:
        logger.debug(f"PreviewManager.load_flipbook called with path: {flipbook_path}")
        directory = Path(flipbook_path)
        if not directory.exists():
            logger.warning(f"Flipbook directory does not exist: {directory}")
            return []

        image_files = sorted(directory.glob('*.png'))  # Adjust the extension if needed
        logger.debug(f"Found {len(image_files)} image files in flipbook directory")
        ...
        logger.debug(f"Loaded {len(frames)} valid frames for flipbook")
        return frames
...
class LargePreviewWindow(QtWidgets.QDialog):
    ...
    def update_flipbook_frame(self):
        if self.flipbook_frames:
            ...
            if num_frames > 0:
                ...
                self.flipbook_preview.setPixmap(self.flipbook_frames[self.current_frame])
            else:
    ...
    def clear_flipbook(self):
        logger.debug("clear_flipbook called")
...
class MyShelfToolUI(QtWidgets.QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.MAX_PREVIEW_SIZE = QtCore.QSize(400, 300)  # You can adjust these values as needed
        ...
        self.preview_manager = PreviewManager(self.MAX_PREVIEW_SIZE)
        ...
        # Flipbook preview
        self.flipbook_label = self.create_styled_label("Flipbook Preview:")
        self.flipbook_preview = QtWidgets.QLabel("No flipbook available")
        ...
        # Snapshot preview
        self.snapshot_label = self.create_styled_label("Snapshot Preview:")
        self.snapshot_preview = QtWidgets.QLabel("No snapshot available")
        self.snapshot_preview.setAlignment(QtCore.Qt.AlignCenter)
        self.snapshot_preview.setFixedSize(self.MAX_PREVIEW_SIZE)
        self.snapshot_new_button = QtWidgets.QPushButton("New Snapshot")
        self.snapshot_new_button.clicked.connect(self.save_snapshot)
        ...
        self.snapshot_delete_button.setEnabled(False)
    ...
    def update_snapshot_preview(self, snapshot):
        logger.debug("update_snapshot_preview called")
        if snapshot and not snapshot.isNull():
            self.snapshot_preview.setPixmap(snapshot)
            self.snapshot_preview.setFixedSize(snapshot.size())
            self.snapshot_delete_button.setEnabled(True)
            logger.debug(f"Snapshot preview updated with size: {snapshot.size()}")
        else:
            logger.warning("Invalid snapshot to display")
            self.clear_snapshot()
    ...
    async def load_preview_assets_async(self, json_data: Dict):
        logger.debug(f"load_preview_assets_async called with json_data: {json_data}")
        try:
            async with self.preview_semaphore:
                ...
                if flipbook_path:
                    ...
                    full_flipbook_path = self.file_manager.get_preview_base_path(self.user_combo.currentText()) / flipbook_path
                    logger.debug(f"Full flipbook path: {full_flipbook_path}")
                    if full_flipbook_path.exists():
                        flipbook_frames = await self.load_flipbook_async(str(full_flipbook_path))
                        logger.debug(f"Loaded {len(flipbook_frames)} flipbook frames")
                        self.update_flipbook_preview(flipbook_frames)
                    else:
                        logger.warning(f"Flipbook path does not exist: {full_flipbook_path}")
                        self.clear_flipbook()

                if snapshot_path:
                    logger.debug(f"Loading snapshot from: {snapshot_path}")
                    full_snapshot_path = self.file_manager.get_preview_base_path(self.user_combo.currentText()) / snapshot_path
                    logger.debug(f"Full snapshot path: {full_snapshot_path}")
                    if full_snapshot_path.exists():
                        snapshot = await self.load_snapshot_async(str(full_snapshot_path))
                        logger.debug(f"Loaded snapshot: {snapshot is not None}")
                        self.update_snapshot_preview(snapshot)
                    else:
                        logger.warning(f"Snapshot path does not exist: {full_snapshot_path}")
                        self.clear_snapshot()

        except Exception as e:
            logger.error(f"Error in load_preview_assets_async: {e}", exc_info=True)
    ...
    async def load_flipbook_async(self, flipbook_path: str) -> List[QtGui.QPixmap]:
        logger.debug(f"load_flipbook_async called with path: {flipbook_path}")
        loop = asyncio.get_event_loop()
        try:
            frames = await loop.run_in_executor(self.thread_pool, self.preview_manager.load_flipbook, flipbook_path)
            logger.debug(f"Loaded {len(frames)} frames from flipbook")
            return frames
        except Exception as e:
            logger.error(f"Error loading flipbook: {e}", exc_info=True)
            return []
    ...
    def start_flipbook_animation(self, frames):
        logger.debug(f"start_flipbook_animation called with {len(frames)} frames")
        if not hasattr(self, 'flipbook_timer'):
            self.flipbook_timer = QtCore.QTimer(self)
            self.flipbook_timer.timeout.connect(self.update_flipbook_frame)
        
        self.flipbook_frames = frames
        self.current_frame = 0
        self.flipbook_timer.start(1000 // 24)  # 24 fps
        logger.debug("Flipbook animation started")

    def update_flipbook_frame(self):
        if self.flipbook_frames:
            self.current_frame = (self.current_frame + 1) % len(self.flipbook_frames)
            self.flipbook_preview.setPixmap(self.flipbook_frames[self.current_frame])
    ...
    def update_flipbook_preview(self, flipbook_frames):
        logger.debug(f"update_flipbook_preview called with {len(flipbook_frames)} frames")
        if flipbook_frames:
            pixmap = flipbook_frames[0]
            self.flipbook_preview.setPixmap(pixmap)
            self.flipbook_preview.setFixedSize(pixmap.size())
            self.start_flipbook_animation(flipbook_frames)
            self.flipbook_delete_button.setEnabled(True)
            logger.debug(f"Flipbook preview updated with size: {pixmap.size()}")
        else:
            logger.warning("No flipbook frames to display")
            self.clear_flipbook()
        ...
        try:
            ...
            if current_file_path:
                ...
                if items:
                    self.file_list_widget.setCurrentItem(items[0])
                    self.file_list_widget.scrollToItem(items[0])
                    self.update_preview(current_file_path)
                else:
                    self.clear_preview()
            else:
                self.clear_preview()
