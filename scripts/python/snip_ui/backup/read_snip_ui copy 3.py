...
file_handler = logging.FileHandler('my_shelf_tool_ui.log')
...
class MyShelfToolUI(QtWidgets.QWidget):
    ...
    def __init__(self, parent=None):
        ...
        logger.info("Loading JSON data and updating file list")
        self.load_json_data()
        self.update_file_list()

        logger.info("Initializing caches and thread pool")
        self.json_cache = {}
        self.preview_cache = {}
        self.thread_pool = ThreadPoolExecutor(max_workers=4)

        logger.info("Setting up progress dialog and timers")
        ...
        logger.info("Setting up loading animation")
        self.loading_movie = QMovie(":/icons/loading_spinner.gif")
        self.loading_movie.setScaledSize(QtCore.QSize(32, 32))

        logger.info("Configuring file list widget")
        ...
        logger.info("Setting up custom sorting")
        self.setup_custom_sorting(self.file_list_widget)

        logger.info("MyShelfToolUI initialization complete")
    ...
    def closeEvent(self, event):
        MyShelfToolUI.instances.remove(self)
        self.save_settings()
    ...
    def setup_custom_sorting(self, tree_widget):
        try:
            logger.info("Starting setup_custom_sorting")
            
            if not isinstance(tree_widget, QtWidgets.QTreeWidget):
                logger.error(f"Invalid tree_widget type: {type(tree_widget)}")
                return
            ...
            logger.info(f"Tree widget column count: {column_count}")
            
            if column_count > 0:
                for i in range(column_count):
                    try:
                        tree_widget.header().setSectionResizeMode(i, QtWidgets.QHeaderView.ResizeToContents)
                    except Exception as e:
                        logger.error(f"Error setting resize mode for column {i}: {str(e)}")
            else:
                logger.warning("Tree widget has no columns")
            ...
            logger.info("Finished setup_custom_sorting")
        except Exception as e:
            logger.error(f"Error in setup_custom_sorting: {str(e)}")
...
def show_my_shelf_tool_ui():
    try:
        ...
        # Check if a QApplication instance exists
        ...
        logger.info("MyShelfToolUI instance created")
        window.show()
        logger.info("MyShelfToolUI shown")
        window.raise_()
        logger.info("MyShelfToolUI raised")
        
        # If we created a new QApplication, start the event loop
